(async function () {
  ...
  eventIdFromUrl = eventIdFromUrl.includes("/")
    ? eventIdFromUrl.split("/")[0]
    : eventIdFromUrl;
  eventID = eventIdFromUrl.includes("?")
    ? eventIdFromUrl.split("?")[0]
    : eventIdFromUrl;

  let vgEventInfoExtracted = document.querySelector(
  ...
  const eventVenueCity =
    vgEventInfoExtractedJSON.location.address.addressLocality;
  const eventVenueState =
    vgEventInfoExtractedJSON.location.address.addressRegion;
  const eventStartDate =
    new Date(vgEventInfoExtractedJSON.startDate).toDateString() +
    " " +
    new Date(vgEventInfoExtractedJSON.startDate).toLocaleTimeString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
    });
  ...
  chrome.runtime.sendMessage({
    eventInfo: vgEventInfoExtractedJSON,
    listingData: JSON.parse(allVgActiveListings.data),
    eventId: eventID,
    type: "stubhubSave",
    status: true,
  });
...
async function getCurrentListings(pageUrl) {
  ...
  requestUrl =
    requestUrl +
    `?ShowAllTickets=true&Quantity=0&PageVisitId=${visitId()}&PageSize=99999&CurrentPage=1&SortBy=ROW&PriceRange=0-NaN`;
