(async function () {
  ...
  let shEventInfoExtracted = document.querySelector(
  ...
  const eventVenueCity =
    shEventInfoExtractedJSON.location.address.addressLocality;
  const eventVenueState =
    shEventInfoExtractedJSON.location.address.addressRegion;
  const eventStartDate =
    new Date(shEventInfoExtractedJSON.startDate).toDateString() +
    " " +
    new Date(shEventInfoExtractedJSON.startDate).toLocaleTimeString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
    });
  ...
  try {
    ...
    indexData = String(document.body.innerHTML).split(
    ...
    try {
      ...
      indexData = String(indexData.data).split(
  ...
  chrome.runtime.sendMessage({
    data: {
      eventInfo: shEventInfoExtractedJSON,
      listingData: JSON.parse(allShActiveListings.data),
      relLink,
      eventId: eventID,
      indexData,
    },
...
async function getCurrentListings(pageUrl) {
  ...
  requestUrl =
    requestUrl +
    `?ShowAllTickets=true&HideDuplicateTicketsV2=false&Quantity=0&PageVisitId=${visitId()}&PageSize=99999&CurrentPage=1&SortBy=ROW&Sections=&TicketClasses=&ListingNotes=&TicketTypeGroups=&PriceRange=0,100&InstantDelivery=false&EstimatedFees=false&Method=IndexSh`;
